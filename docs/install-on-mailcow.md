...
The required Mailcow Agent is reached directly over Docker DNS in the same-host deployment:

```dotenv
MOOLIAS_MAILCOW_AGENT_URL=http://moolias-agent:8081
```

If Newsletter Management is installed, its restricted Agent likewise uses the direct Docker-network route:

```dotenv
MOOLIAS_NEWSLETTER_AGENT_URL=http://moolias-newsletter-agent:8082
```

Both Agents still have their authenticated Mailcow-nginx locations for a Moolias application running on another host. Standalone deployments may leave `MAILCOW_INTERNAL_URL` empty; Moolias then falls back to `MAILCOW_URL` for backend requests. A standalone Moolias application still needs the Mailcow Agent installed on the Mailcow host; an empty `MOOLIAS_MAILCOW_AGENT_URL` falls back to `<MAILCOW_URL>/moolias-agent`.
...
- keeps the authenticated Mailcow-nginx endpoint available for remote/standalone access, without a published host port;
- configures the same-host Moolias application to use `http://moolias-newsletter-agent:8082` directly on the Mailcow Docker network;
