...
def test_first_delivery_bypass_uses_exact_dynamic_recipient_map():
    assert "MOOLIAS_AGENT_BYPASS_MAP_PATH=/rspamd-custom/moolias_firstmail_recipients.map" in INSTALLER
    assert "./data/conf/rspamd/custom/moolias-sender-agent:/rspamd-custom" in INSTALLER
    assert "file:///etc/rspamd/custom/moolias-sender-agent/moolias_firstmail_recipients.map" in INSTALLER
...
def test_rspamd_restart_restores_agent_map_permissions_and_validates_config():
    ...
    assert "chown 10001:10001 /etc/rspamd/custom/moolias-sender-agent/moolias_firstmail_recipients.map" in INSTALLER
    assert "docker compose exec -T rspamd-mailcow rspamadm configtest" in INSTALLER
    assert "docker compose exec -T moolias-sender-agent test -w /rspamd-custom" in INSTALLER
